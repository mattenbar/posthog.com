...
### PR base branch overrides

Auto-started inbox PRs target each repository's default branch. To target a different branch (like `staging` or `develop`), add per-repo overrides in **PR base branch overrides**. Enter the repository in `organization/repository` format (e.g., `acme/web`) and specify the target branch.
